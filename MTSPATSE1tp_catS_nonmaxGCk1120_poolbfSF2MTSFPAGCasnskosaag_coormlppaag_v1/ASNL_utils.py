...
class pointASNLall(nn.Module):
    def __init__(self,in_channel,b_c,b_c1,mlp_list,l_mlp_list,NL=True):
        super(pointASNLall,self).__init__()
        self.b_c = b_c
        self.b_c1 = b_c1
        self.NL = NL
        ...
        self.lq_conv = nn.Conv2d(in_channel,l_mlp_list[-1],1,bias=False)
        self.lkv_conv = nn.Conv2d(in_channel,l_mlp_list[-1]*2,1,bias=False)
        
        self.l1q_conv = nn.Conv2d(in_channel,l_mlp_list[-1],1,bias=False)
        self.l1k_conv = nn.Conv2d(in_channel,l_mlp_list[-1],1,bias=False)
        ...
        self.bconv = nn.Conv2d(in_channel,1,kernel_size=1,bias=False)
        self.bbn = nn.BatchNorm2d(1)
        self.bvconv = nn.Conv2d(in_channel,in_channel,kernel_size=1,bias=False)
        self.bvbn = nn.BatchNorm2d(in_channel)
        self.bww = nn.Conv2d(in_channel*2+2,1,kernel_size=1,bias=False)
        self.bwbn = nn.BatchNorm2d(1)
        self.wga01 = nn.Conv2d(in_channel*2+3,1,kernel_size=1,bias=False)
    ...
    def forward(self,xyz,points):
        ...
        grouped_points = torch.cat([xyz,points],dim=-1).view(B,1,N,-1)#[B,1,N,C]
        ...
        new_weight_xyz = new_group_weight[:,:,:,0].view(B,N,1)
        #print("new_weight_xyz is:",new_weight_xyz)
        new_weight_feature = new_group_weight[:,:,:,1:].view(B,N,-1)
        #print("new_weight_feature is:",new_weight_feature.shape)
        
        new_xyz = torch.sum(new_weight_xyz * xyz,dim=1,keepdim=True)#[B,1,C]
        new_feature = torch.sum(new_weight_feature * points,dim=1,keepdim=True)#[B,1,D]
        
        new_xyz = new_xyz.permute(0,2,1)#[B,C,S]
        #print("new_xyz is:",new_xyz.shape)
        new_feature = new_feature.permute(0,2,1)#[B,C,S]
        #print("new_feature is:",new_feature.shape)
        
        new_group_feature = torch.cat([xyz,points],dim=-1).view(B,1,N,-1)#[B,1,N,C]
        #print("new_group_feature is:",new_group_feature.shape)
        new_c_feature = torch.cat([new_xyz.permute(0,2,1),new_feature.permute(0,2,1)],dim=-1).view(B,1,1,-1)#[B,1,1,C]
        #print("new_c_feature is:",new_c_feature.shape)
        
        new_points = torch.cat([new_group_feature - new_c_feature,new_c_feature.repeat(1,1,N,1)],dim=-1)
        ...
        """local"""
        Ql = self.lq_conv(new_c_feature.permute(0,3,1,2))#[B,C,S,1]
        #print("Ql is:",Ql.shape)
        KVl = self.lkv_conv(new_group_feature.permute(0,3,1,2))#[B,2*C,S,K]
        
        Ql1 = self.l1q_conv(new_c_feature.permute(0,3,1,2))#[B,C,S,1]
        #print("Ql is:",Ql.shape)
        Kl1 = self.l1k_conv(new_group_feature.permute(0,3,1,2))#[B,C,S,K]
        ...
        position = (new_group_feature - new_c_feature).permute(0,3,1,2)#[B,C,S,K]
        ...
        diff = new_group_feature - new_c_feature
        ...
        basevactor = torch.mean(basevactor,dim=3).view(B,-1,1,1)#[B,C,N,1]
        ...
        inn_product = basevactor.matmul(diff.permute(0,1,3,2)).view(B,1,-1)
        #print("inn_product is:",inn_product.shape)
        cos = (inn_product / torch.sqrt((distent1 * distent2) + 1e-10)).view(B,1,N,1)
        #print("cos is:",cos.shape)
        diff = F.leaky_relu(self.bbn(self.bconv(diff.permute(0,3,1,2))),0.2).permute(0,2,3,1)#[B,N,k,1]
        agg_info1 = torch.cat((new_group_feature,f_abs,cos,diff),dim=3)#[B,N,k,C]
        t = agg_info1.permute(0,3,1,2)#[B,C,N,k]
        t = F.leaky_relu(self.bwbn(self.bww(t)),0.2).permute(0,2,3,1)#[B,N,k,1]
        agg_info1 = torch.cat((new_group_feature,f_abs,cos,diff,t),dim=3)#[B,N,k,C]
        agg_info1 = self.wga01(agg_info1.permute(0,3,1,2))#[B,C,S,K]
        #print("agg_info1 is:",agg_info1.shape)
        
        att = QKl + position + agg_info1
        att = F.relu(self.bnatw(self.atw(att)))
        att = F.softmax(self.atw1(att)/(self.l_mlp_list[-1]**0.5),dim=-1)#[B,1,S,K] 
        new_points = att * (Vl + position + agg_info1)
        new_points = torch.sum(new_points,dim=-1,keepdim=True)#[B,C,S,1]
        new_points = F.relu(self.pc_bn(self.pc_conv(new_points)))#[B,D,S,1]
        #print("new_points is:",new_points.shape)
        new_points = new_points.view(B,-1,1)
...
class pointASNL(nn.Module):
    def __init__(self,npoint,nsample,in_channel,b_c,b_c1,mlp_list,l_mlp_list,NL=True):
        ...
        self.bconv = nn.Conv2d(3,1,kernel_size=1,bias=False)
        self.bbn = nn.BatchNorm2d(1)
        self.bvconv = nn.Conv2d(3,3,kernel_size=1,bias=False)
        self.bvbn = nn.BatchNorm2d(3)
        self.bww = nn.Conv2d(3+1,1,kernel_size=1,bias=False)
        self.bwbn = nn.BatchNorm2d(1)
        self.wga01 = nn.Conv1d(3+2+in_channel-3,in_channel-3,kernel_size=1,bias=False)
        self.bnwga01 = nn.BatchNorm1d(in_channel-3)
        
        self.q_conv = nn.Conv2d(in_channel,b_c,1,bias=False)
        self.kv_conv = nn.Conv2d(in_channel,b_c*2,1,bias=False)
        self.sc_conv = nn.Conv2d(in_channel,b_c,1,bias=False)
    ...
    def forward(self,xyz,points,cidx=None,as_neighbor=11):
        ...
        sc = self.sc_conv(grouped_points)
        new_group_points -= sc
        ...
        new_feature = torch.sum(new_weight_feature * index_points(points,group_idx),dim=2)#[B,S,D]
        """Augment sampling"""
        
        """structure_new"""
        group_idx_s = query_ball_point(as_neighbor,xyz,new_xyz)
        grouped_xyz_s = index_points(xyz,group_idx_s)[:,:,1:,:]#[B,S,k,C]
        
        diff = grouped_xyz_s - new_xyz.view(B,S,1,C)
        #print("diff is:",diff.shape)
        f_abs = torch.abs(diff)
        basevactor = F.leaky_relu(self.bvbn(self.bvconv(diff.permute(0,3,1,2))),0.2)#[B,C,S,k]
        basevactor = torch.mean(basevactor,dim=3).view(B,-1,S,1)#[B,C,S,1]
        basevactor = basevactor.permute(0,2,3,1)#[B,S,1,C]
        #print("basevactor is:",basevactor.shape)
        distent1 = torch.sum(basevactor ** 2,dim=3) 
        distent2 = torch.sum(diff ** 2,dim=3) 
        inn_product = basevactor.matmul(diff.permute(0,1,3,2)).view(B,S,-1)
        #print("inn_product is:",inn_product.shape)
        cos = (inn_product / torch.sqrt((distent1 * distent2) + 1e-10)).view(B,S,as_neighbor-1,1)
        #print("cos is:",cos.shape)
        diff = F.leaky_relu(self.bbn(self.bconv(diff.permute(0,3,1,2))),0.2).permute(0,2,3,1)#[B,S,k,1]
        agg_info1 = torch.cat((f_abs,diff),dim=3)#[B,S,k,C]
        t = agg_info1.permute(0,3,1,2)#[B,C,S,k]
        t = F.leaky_relu(self.bwbn(self.bww(t)),0.2).permute(0,2,3,1)#[B,S,k,1]
        agg_info1 = torch.cat((f_abs,diff,t),dim=3)#[B,S,k,C]
        #print("agg_info1 is:",agg_info1.shape)
        agg_info1 = torch.sum(cos * agg_info1,dim=2)#[B,S,C]
        """new"""
        agg_info1 = torch.cat([new_feature,agg_info1],dim=-1)#[B,S,C]
        agg_info1 = F.relu(self.bnwga01(self.wga01(agg_info1.permute(0,2,1))))#[B,C,S]
        #print("agg_info1 is:",agg_info1.shape)
        """structure_new"""
        ...
        """local"""
        in_gc_t = agg_info1.permute(0,2,1).view(B,S,1,-1)
        #print("new_points is:",new_points.shape)
...
print(batch1.shape)
#t = query_ball_point(3,batch1.transpose(2,1),batch1.transpose(2,1))
#print(t) 
#group = index_points(batch1.transpose(2,1),t)
#print("group is:",group)  
#sa = PointNetSetAbstractionMsg(2,3,in_channel=3+3,b_c=32,b_c1=32,mlp_list=[32,3+1],l_mlp_list=[32,64],NL=False)
#xyz,r = sa(batch1,batch1)
#print("xyz is:",xyz)
#print("r is:",r.shape)
sa1 = pointASNL(2,3,in_channel=3+3,b_c=32,b_c1=32,mlp_list=[32,3+1],l_mlp_list=[32,64],NL=False)
xyz1,r1,pool_idx = sa1(batch1,batch1,as_neighbor=3)
print("xyz1 is:",xyz1.shape)
print("r1 is:",r1.shape)
sa_all = pointASNLall(in_channel=3+3,b_c=32,b_c1=32,mlp_list=[32,3+1],l_mlp_list=[32,64],NL=False)
xyz2,r2 = sa_all(batch1,batch1)
print("xyz2 is:",xyz2.shape)
